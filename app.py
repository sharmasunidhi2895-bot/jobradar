...
def _norm_company(s: str) -> str:
    ...
    s = re.sub(r"\b(inc|incorporated|corp|corporation|co|company|ltd|llc|plc|gmbh|technologies|technology)\b", " ", s)
    s = re.sub(r"\s+", " ", s).strip()
    return s

def _employer_match_score(target_company: str, employer_name: str) -> float:
    """0..1 fuzzy score based on token overlap + substring match."""
    a = _norm_company(target_company)
    b = _norm_company(employer_name)
    if not a or not b:
        return 0.0
    if a in b or b in a:
        return 1.0
...
with st.expander("📄 Upload Resume for AI Match % Scoring (optional)"):
    ...
    if resume_file:
        if resume_file.type == "text/plain":
            ...
            if resume_text:
                st.success(f"✓ Resume PDF parsed — {len(resume_text)} characters")
            else:
                st.warning("📄 Could not extract text from this PDF (it may be a scanned image). Please paste your resume text below.")
...
def search_company(company, keywords, location, rapid_key, date_posted, emp_type, n):
    ...
    - If `company` is provided: uses soft employer matching to prioritize that org.
    - If `company` is empty: searches by designation/keywords only (broad discovery).
    """
    def call_api(params):
        resp = requests.get(
    ...
    # If no company name is provided, do a broad search by keywords/location.
    if not (company or '').strip():
        ...
        if location:    params0["location"] = location
        if date_posted: params0["date_posted"] = date_posted
        if emp_type:    params0["employment_types"] = emp_type
        return call_api(params0)[:n]

    # 1) Targeted query
    query = f"{keywords} at {company}".strip()
    params = {
        "query": query,
        "page": "1",
        "num_pages": "2",
    ...
    all_results = call_api(params)

    # Soft match instead of strict contains()
    scored = []
    for j in all_results:
        emp = j.get("employer_name") or ""
        score = _employer_match_score(company, emp)
        scored.append((score, j))

    strong = [j for s, j in scored if s >= 0.5]
    if strong:
        # Prefer employer matches first
        strong_sorted = sorted(strong, key=lambda j: _employer_match_score(company, j.get("employer_name") or ""), reverse=True)
        return strong_sorted[:n]

    # 2) Broader search (drop date_posted), keep location
    params2 = {
    ...
    if location: params2["location"] = location
    if emp_type: params2["employment_types"] = emp_type

    all_results2 = call_api(params2)

    scored2 = []
    for j in all_results2:
        emp = j.get("employer_name") or ""
        score = _employer_match_score(company, emp)
        scored2.append((score, j))

    # Return best matches if any, else return top results (so UI doesn't look broken)
    scored2.sort(key=lambda t: t[0], reverse=True)
    best = [j for s, j in scored2 if s > 0]
    return (best[:n] if best else all_results2[:n])
...
if unique:
    ...
    for job in unique:
        ...
        with st.container(border=True):
            ...
            with btn_col2:
                # Save/unsave this job
                job_key = str(job.get('job_id') or job.get('job_apply_link') or job.get('job_google_link') or job.get('job_title',''))
                is_saved = job_key in st.session_state.get('saved_jobs', {})
                if st.button('★ Saved' if is_saved else '☆ Save', key=f"save_{job_key}"):
                    if is_saved:
                        st.session_state['saved_jobs'].pop(job_key, None)
                    else:
                        st.session_state['saved_jobs'][job_key] = {
