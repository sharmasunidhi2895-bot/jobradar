...
# ── Secrets / API Key ─────────────────────────────────────────────────────────
# Store RAPIDAPI_KEY in Streamlit Cloud → App → Settings → Secrets (do NOT hardcode in code)
try:
    rapid_key = st.secrets["RAPIDAPI_KEY"]
except Exception:
    st.error("Missing RAPIDAPI_KEY. Add it in Streamlit Secrets (Settings → Secrets).")
    st.stop()
...
# ── Search button ──────────────────────────────────────────────────────────────
st.session_state['companies_raw'] = companies_raw
...
def search_company(company, keywords, location, rapid_key, date_posted, emp_type, n):
    """Search jobs via RapidAPI JSearch. Uses soft employer matching to avoid returning 0 jobs."""
    def call_api(params):
        resp = requests.get(
...
if search_clicked:
    if not companies_raw.strip() and not keywords.strip():
        st.error("⚠️ Please enter a company OR job title.")
        st.stop()
    ...
    for i, company in enumerate(companies):
        ...
        try:
            jobs = search_company(company or '', keywords, location, rapid_key, date_posted, emp_type, results_per_co)
