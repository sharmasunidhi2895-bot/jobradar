...
if "api_debug" not in st.session_state:
    st.session_state["api_debug"] = []  # list of meta dicts from API calls
...
def jsearch(query: str, location: str, date_posted: str, employment_type: str, limit: int, num_pages: int = 5) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
    Fetch jobs from RapidAPI JSearch.

    Notes:
    - Many plans return ~10 jobs per page. If you only fetch 1 page, you often see ~10 results.
    - We therefore fetch multiple pages (default 5) and then trim to `limit`.
    Returns: (jobs, meta)
    """
    ...
    params = {
        ...
        "num_pages": str(max(1, min(10, int(num_pages)))),
    }

    # Optional filters (some plans may ignore silently)
    if date_posted and date_posted != "any":
        params["date_posted"] = date_posted  # today, 3days, week, month
    if employment_type and employment_type != "any":
        params["employment_types"] = employment_type  # FULLTIME, PARTTIME, CONTRACTOR, INTERN

    r = requests.get(url, headers=headers, params=params, timeout=30)
    meta: Dict[str, Any] = {
        "status_code": r.status_code,
        "requested_num_pages": params["num_pages"],
        "query": query,
    ...
    try:
        ...
        meta["response_text_head"] = (r.text or "")[:300]
        return [], meta

    if isinstance(data, dict):
        if "message" in data:
            meta["message"] = data.get("message")
        if "status" in data:
    ...
    meta["returned_count"] = len(jobs)

    # Trim to limit (UI "Results per search")
    if limit:
        jobs = jobs[: int(limit)]
...
def match_score_keywords(resume_text: str, jd_text: str) -> Tuple[int, List[str], List[str]]:
    """Fallback keyword-based scorer when no Groq key available."""
    ...
    for label, pats in groups.items():
        ...
        if in_jd and in_resume:
            score += 14; highlights.append(label)
...
def match_score_ai(resume_text: str, jd_text: str, groq_key: str) -> Tuple[int, List[str], List[str]]:
    """AI-powered scoring via Groq (free). Returns (score, highlights, gaps)."""
    prompt = f"""You are a resume match expert. Score this resume against this job description.

Return ONLY a JSON object, no markdown, no explanation:
{{"score": <0-100>, "highlights": ["strength1", "strength2", "strength3"], "gaps": ["gap1", "gap2"], "verdict": "one honest sentence"}}
    ...
    try:
        ...
        if m:
            ...
            return (
                ...
                data.get("highlights", [])[:6],
                data.get("gaps", [])[:4],
            )
    except Exception:
        pass
    return match_score_keywords(resume_text, jd_text)

def match_score(resume_text: str, jd_text: str, groq_key: str = "") -> Tuple[int, List[str], List[str]]:
    """Use AI if Groq key available, else keyword fallback."""
    if groq_key:
        return match_score_ai(resume_text, jd_text, groq_key)
    return match_score_keywords(resume_text, jd_text)
...
.hero-badge {
    ...
    font-size: 12px; font-weight: 600; color: #a89dff;
...
.hero-title {
    ...
    background: linear-gradient(135deg, #ffffff 30%, #a89dff 70%, #6c63ff);
...
<div class="jobradar-hero">
    <div class="hero-badge">✦ AI-Powered Job Intelligence</div>
    <div class="hero-title">🎯 JobRadar</div>
    <div class="hero-sub">Stop scrolling job boards. Track the companies you want, get fresh postings the moment they drop — matched to your resume.</div>
    <div class="hero-pills">
        <span class="hero-pill">⚡ Real-time postings</span>
        <span class="hero-pill">🎯 Resume match scoring</span>
        <span class="hero-pill">⭐ Save & track jobs</span>
        <span class="hero-pill">📅 Date filters</span>
    </div>
</div>
...
if not RAPIDAPI_KEY:
    RAPIDAPI_KEY = st.text_input("🔑 RapidAPI Key", type="password",
        ...
        help="Free at rapidapi.com → search JSearch → Subscribe Basic (200 req/month free)")
    if not RAPIDAPI_KEY:
        st.info("👆 Add your free RapidAPI key above to start searching. [Get it here](https://rapidapi.com/letscrape-6bRBa3QguO5/api/jsearch)")
        st.stop()

# Groq API key (free AI for resume match scoring)
try:
...
if not GROQ_KEY:
    with st.expander("🤖 Add free AI resume match scoring (optional)", expanded=False):
        st.markdown("""
...
3. Paste it below — stored only in your browser session
        """)
        GROQ_KEY = st.text_input("Groq API Key (free)", type="password",
            placeholder="gsk_…", key="groq_key_input")
        if GROQ_KEY:
            st.session_state["groq_key"] = GROQ_KEY
            st.success("✅ AI resume matching enabled!")

if not GROQ_KEY and "groq_key" in st.session_state:
...
if search_clicked:
    ...
    for i, company in enumerate(companies):
        ...
        jobs, meta = jsearch(query=query, location=location, date_posted=date_posted, employment_type=employment_type, limit=limit, num_pages=max(5, (limit + 9)//10))

        st.session_state["api_debug"].append(meta)
    ...
    with st.expander("🔎 API debug (why results might be low)", expanded=False):
        if not st.session_state["api_debug"]:
            st.write("No API calls recorded.")
        else:
            for md in st.session_state["api_debug"]:
                st.write(md)
        ...
        if resume_text:
            if jid in st.session_state["match_cache"]:
                ...
                if st.button("🎯 Match Resume", key=f"match_{jid}"):
                    score, hi, gaps = match_score(resume_text, job_desc(j), GROQ_KEY)
