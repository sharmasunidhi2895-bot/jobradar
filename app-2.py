...
def search_company(company, keywords, location, rapid_key, date_posted, emp_type, n):
    ...
    params = {
        ...
        "page": "1",
        "num_pages": "1",
        "results_per_page": str(n * 3),  # fetch more, then filter
        "country": "us",
    }
    ...
    # Filter to only jobs where employer name matches the company
    company_lower = company.lower()
    filtered = [
        j for j in all_results
        if company_lower in (j.get("employer_name") or "").lower()
    ]

    # If strict filter returns nothing, fall back to all results
    return filtered[:n] if filtered else all_results[:n]
